...
def class_accuracy(pred_labels, gt_labels, return_FP=False, max_classes=3):
    ...
    class_counts = np.zeros(max_classes, dtype=int)
    correct_pred_counts = np.zeros(max_classes, dtype=int)

    if return_FP:
        FP_pred_counts = np.zeros(max_classes, dtype=int)
    ...
    for i, c in enumerate(unique_classes):
        ...
        if return_FP:
            FP_pred_counts[i] = np.sum((pred_labels == c) & (~class_mask))

    if return_FP:
        return class_counts, correct_pred_counts, FP_pred_counts
    else:
        return class_counts, correct_pred_counts


def resize_image_and_labels(X, Y, output_size):
    """
    Resize an image and its corresponding labels.

    Parameters:
    - X: The input image, assumed to be of size [height, width, channels].
    - Y: The corresponding label image, assumed to be of size [height, width].
    - output_size: The desired output size, in the format (new_height, new_width).

    Returns:
    - X_resized: The resized image.
    - Y_resized: The resized labels, converted back from one-hot encoding.
    """
    ...
    def one_hot_to_labels(one_hot):
        """Convert an image in one-hot encoding format back to label format."""
        return np.argmax(one_hot, axis=-1)

    # Resize the image X
    X_resized = np.zeros((output_size[0], output_size[1], X.shape[2]))
    for i in range(X.shape[2]):
        X_resized[:, :, i] = cv2.resize(X[:, :, i], output_size, interpolation=cv2.INTER_AREA)

    # Convert Y to one-hot encoding, resize, and then convert back
    Y_one_hot = convert_to_one_hot(Y)
    Y_resized_one_hot = np.zeros((output_size[0], output_size[1], Y_one_hot.shape[2]))
    for i in range(Y_one_hot.shape[2]):
        Y_resized_one_hot[:, :, i] = cv2.resize(Y_one_hot[:, :, i].astype(float), output_size,
                                                interpolation=cv2.INTER_AREA)

    # Convert the resized one-hot encoded Y back to label format (0, 1, 2)
    Y_resized = one_hot_to_labels(Y_resized_one_hot)
